RSpec.shared_examples_for :related_resources_operations do |namespace|
  ...
  describe "GET #{namespace}/articles/:id/comments" do
    subject(:last_response) { get("#{namespace}/articles/#{article.external_id}/comments") }
    ...
    context 'unauthorized for show_related_resources' do
      before { disallow_operation('show_related_resources', source_record: article, related_record_class: Comment) }
      it { is_expected.to be_forbidden }
    end

    context 'authorized for show_related_resources' do
      before { allow_operation('show_related_resources', source_record: article, related_record_class: Comment) }
  ...
  describe "GET #{namespace}/articles/:id/author" do
    subject(:last_response) { get("#{namespace}/articles/#{article.external_id}/author") }
